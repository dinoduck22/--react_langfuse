# Langfuse (React + Vite + TSX)
### 시작하기
1. git 저장소 다운로드
```shell 
$ git clone https://github.com/dinoduck22/--react_langfuse.git
```
2. 다운로드 받은 파일로 이동
```shell
$ cd --react_langfuse
...
└─src
    ...
    │  ├─LineNumberedTextarea : 코드 블럭 텍스트 상자 (JSX_Tracing 브랜치 에서는 CodeBlock)
    │  ├─Modal : 모달 창
    │  ├─PageHeader : 페이지 헤더부분 (모든 페이지 공통)
    │  ├─SearchInput : 검색 창
