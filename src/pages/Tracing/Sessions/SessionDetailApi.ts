// src/pages/Tracing/SessionDetailApi.ts
import { langfuse } from 'lib/langfuse';
import { SessionWithTraces, UiSessionData, UiTrace, JsonValue } from './sessionDetailTypes';

// API 응답값을 UI에 표시하기 안전한 문자열로 변환
const formatTraceValue = (value: JsonValue | null): string => {
...
const transformDataForUi = (apiData: SessionWithTraces): UiSessionData => {
  const uiTraces: UiTrace[] = apiData.traces.map(trace => {
...
export const fetchSessionDetails = async (sessionId: string): Promise<UiSessionData> => {
    try {
        // ▼▼▼ 오류 수정 ▼▼▼
        // { sessionId } 객체 대신 sessionId 문자열을 직접 전달합니다.
        const response = await langfuse.api.sessionsGet(sessionId);
        const apiData = response as unknown as SessionWithTraces;
