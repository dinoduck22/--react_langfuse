// src/pages/Tracing/TracingApi.ts
import { langfuse } from '../../lib/langfuse';
import { Trace, JsonValue } from './types'; // JsonValue 타입 import

// API 응답 데이터에 대한 타입 정의
interface LangfuseTrace {
  id: string;
  timestamp: string;
  name: string | null;
  input: unknown;
  output: unknown;
  sessionId: string | null;
  release: string | null;
  version: string | null;
  userId: string | null;
  metadata: Record<string, unknown> | null;
  tags: string[];
  public: boolean | null;
  environment: string | null;
  latency: number;
  cost: number;
}

interface TracesApiResponse {
  data: LangfuseTrace[];
  meta: {
    page: number;
    limit: number;
    totalItems: number;
    totalPages: number;
  };
}

/**
 * API 응답 값을 UI에 표시하기 안전한 문자열로 변환합니다.
 * @param value 변환할 값
 */
const formatTraceValue = (value: unknown): string => {
    if (value === null || typeof value === 'undefined') {
        return 'N/A';
    }
    if (typeof value === 'string') {
        return value;
    }
    return JSON.stringify(value, null, 2);
}

/**
 * Langfuse API에서 트레이스 목록을 가져옵니다.
 */
export const fetchTraces = async (): Promise<Trace[]> => {
  try {
    const response = await langfuse.api.traceList({});
    const apiResponse = response as unknown as TracesApiResponse;

    return apiResponse.data.map(trace => ({
      id: trace.id,
      timestamp: new Date(trace.timestamp).toLocaleString(),
      name: trace.name ?? 'N/A',
      input: formatTraceValue(trace.input),
      output: formatTraceValue(trace.output),
      sessionId: trace.sessionId ?? 'N/A',
      userId: trace.userId ?? 'N/A',
      release: trace.release ?? 'N/A',
      version: trace.version ?? 'N/A',
      tags: trace.tags ?? [],
      isFavorited: false,
      observations: 0,
      env: trace.environment ?? 'default',
      latency: trace.latency ?? 0,
      cost: trace.cost ?? 0,
      public: trace.public,
      // metadata 타입을 JsonValue로 단언하여 타입 에러 해결
      metadata: trace.metadata as Record<string, JsonValue> | null,
      environment: trace.environment
    }));
  } catch (error) {
    console.error("Failed to fetch traces:", error);
    throw new Error('트레이스 목록을 불러오는 데 실패했습니다.');
  }
};